...
IP = '178.18.244.121' # IP address of the server
        ...
        with conn:
            ...
            while True:
                ...
                if not remote_payment_request: # break out of while loop in case no data is sent
                    ...
                    break
                print ('\n', '\n', 'initial balance: ', lightning_stub.ChannelBalance(lnrpc.ChannelBalanceRequest(), metadata=[('macaroon', macaroon)]).local_balance.sat) # displays the sum of local
                ...
                for response in router_stub.SendPaymentV2(request, metadata=[('macaroon', macaroon)]): # send the payment
                    print(response)
                print ('\n', 'Conducted payment')
                print ('\n', 'updated balance: ', lightning_stub.ChannelBalance(lnrpc.ChannelBalanceRequest(), metadata=[('macaroon', macaroon)]).local_balance.sat) # displays the sum of local
                ...
                conn.sendall(own_payment_request.encode('UTF-8')) # send payment request over socket
