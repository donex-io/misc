...
start_ln() {
	...
	echo This is the Bitcoin core path: "$PATH_TO_BITCOIN"

	# Create config file
	cat <<- EOF > "$PATH_TO_BITCOIN/bitcoin.conf"
	regtest=1
	txindex=1
	server=1
	...
	test -f "$PATH_TO_BITCOIN/regtest/bitcoind.pid" || \
		bitcoind #modified to use config file, old arguments: -regtest -txindex -fallbackfee=0.00000253 -daemon
